import sys

import pytest

sys.path.insert(0, "")

from check_models import (
    ProviderConfig,
    OpenAICompatibleProvider,
    ModelResult,
    run_checks,
)


class DummyResp:
    def __init__(self, status_code=200, json_data=None, text="", iter_lines=None):
        self.status_code = status_code
        self._json = json_data
        self.text = text
        self._iter_lines = iter_lines

    def json(self):
        if self._json is None:
            raise ValueError("no json")
        return self._json

    def raise_for_status(self):
        if self.status_code >= 400:
            raise Exception(f"HTTP {self.status_code}")

    def iter_lines(self, decode_unicode=True):
        if self._iter_lines is None:
            return iter(())
        return iter(self._iter_lines)

    def __enter__(self):
        return self

    def __exit__(self, exc_type, exc, tb):
        return False


def make_session(get_resp=None, post_resp=None):
    class S:
        def get(self, url, headers=None, timeout=None):
            return get_resp

        def post(self, url, headers=None, json=None, timeout=None, stream=False):
            return post_resp

        def mount(self, prefix, adapter):
            return None

    return S()


def test_run_checks_plain_success(monkeypatch):
    cfg = ProviderConfig(name="p", type="openai", base_url="https://x", api_key="k")
    get_resp = DummyResp(status_code=200, json_data={"data": [{"id": "m1"}, {"id": "m2"}]})
    post_resp = DummyResp(status_code=200, json_data={"choices": [{"message": {"content": "OK"}}]})
    session = make_session(get_resp, post_resp)
    monkeypatch.setattr('check_models.requests.Session', lambda: session)

    p = OpenAICompatibleProvider(cfg, timeout=5, test_prompt="hi", max_tokens=2)
    p.session = session

    results = run_checks([p], concurrency=1)

    assert len(results) == 2
    assert all(isinstance(r, ModelResult) for r in results)
    assert all(r.status == "WORKING" for r in results)


def test_run_checks_streaming_ttft(monkeypatch):
    cfg = ProviderConfig(name="p", type="openai", base_url="https://x", api_key="k", measure_ttft=True)
    get_resp = DummyResp(status_code=200, json_data={"data": [{"id": "m1"}]})
    lines = ["data: {\"choices\": [{\"delta\": {}}]}", "data: {\"choices\": [{\"delta\": {\"content\": \"h\"}}]}", "data: [DONE]"]
    post_resp = DummyResp(status_code=200, iter_lines=lines)
    session = make_session(get_resp, post_resp)
    monkeypatch.setattr('check_models.requests.Session', lambda: session)

    p = OpenAICompatibleProvider(cfg, timeout=5, test_prompt="hi", max_tokens=2)
    p.session = session

    results = run_checks([p], concurrency=1)

    assert len(results) == 1
    assert results[0].status == "WORKING"
    assert results[0].ttft_ms is not None
